<template>
  <div class="herb-detail-container">
    ...
    <el-card v-loading="loading" element-loading-text="正在加载蛋白质详情...">
      <div slot="header" class="card-header">
        <span class="card-title">蛋白质详情</span>
      ...
      <div class="section">
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12"><p><strong>Gene 名称：</strong>{{ detail.geneName || '--' }}</p></el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="12"><p><strong>Gene Alias：</strong>{{ (detail.geneAlias || []).join('、') || '--' }}</p></el-col>
          <el-col :span="12"><p><strong>染色体位置：</strong>{{ detail.chromosome || '--' }}</p></el-col>
        </el-row>
        <el-row :gutter="20">
        ...
        <el-row :gutter="20">
          <el-col :span="12"><p><strong>描述：</strong>{{ detail.description || '--' }}</p></el-col>
          <el-col :span="12"><p><strong>基因类型：</strong>{{ detail.typeOfGene || '--' }}</p></el-col>
        </el-row>
      </div>
      ...
      request.get(`/api/protein/detail/${id}`)
          .then(res => {
            if (res.code === '200') {
              ...
              this.$message.error(res.msg || '获取蛋白质详情失败')
            }
          })
          .catch(err => {
