    ...
    <el-card>
      <template #header>
        <div class="header-wrapper">
          <span class="title">成分信息</span>

          <div class="title-search">
            <div class="search-bar">
              <el-input
                  v-model="searchKeyword"
                  placeholder="输入 ID、名称、CAS、分子式或 SMILES"
                  size="small"
                  clearable
            ...
            <div class="tip-container">
              <ul class="search-tip">
                <li>成分ID 或 成分名称</li>
                <li>CAS号 或 SMILES</li>
                <li>例如：ING000009、523-42-2、Cyanin、CCCCCC=CCC=CCCCCCO</li>
              </ul>
            </div>
          </div>
        </div>
        ...
        <el-table-column prop="cas" label="CAS号" width="150">
          <template #default="scope">
            ...
            <span v-else>
              ...
              <span style="color: #909399; margin-left: 4px;">无CAS号</span>
            </span>
          </template>
        </el-table-column>
    ...
    loadData() {
      request.get('api/ingredient/all').then(res => {
        if (res.code === '200') {
          ...
          this.$message.error(res.msg || '获取成分信息失败')
        }
      })
    },
    doSearch() {
