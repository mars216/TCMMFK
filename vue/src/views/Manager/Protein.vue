    ...
    <el-card>
      <template #header>
        <div class="header-wrapper">
          <span class="title">Protein Information</span>

          <div class="title-search">
            <div class="search-bar">
              <el-input
                  v-model="searchKeyword"
                  placeholder="Enter ID, Protein ID, Name"
                  size="small"
                  clearable
            ...
            <div class="tip-container">
              <ul class="search-tip">
                <li>Keywords for Protein ID, Name or Database Cross Reference</li>
                <li>Example: PRO0047321, Penicillin, OMIM</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      <el-table :data="pagedData" style="width: 100%">
        ...
        <el-table-column prop="name" label="Name">
          <template #default="scope">
            <span>{{ scope.row.name || 'Unknown Protein' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="dbXrefs" label="Database Reference">
          <template #default="scope">
    ...
    loadData() {
      request.get('/api/protein/all').then(res => {
        if (res.code === '200') {
          ...
          this.$message.error(res.msg || 'Failed to get protein information')
        }
      })
    },
    doSearch() {
